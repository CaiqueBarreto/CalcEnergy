...
def adicionar_dispositivo(sistema):
    ...
    # 1. Primeiro declaramos todos os widgets
    nome_entry = ctk.CTkEntry(janela)
    potencia_entry = ctk.CTkEntry(janela)
    resultado_label = ctk.CTkLabel(janela, text="")
    consumo_label = ctk.CTkLabel(janela, text="", font=("Arial", 10))

    # 2. Depois definimos as funções internas
    def pesquisar_consumo():
        nome = nome_entry.get().strip()
        if nome:
            consumo_label.configure(text="Pesquisando consumo médio...", text_color="yellow")
            janela.update()
            
            consumo = pesquisar_consumo_equipamento(nome)
            if consumo:
                potencia_entry.delete(0, 'end')
                potencia_entry.insert(0, str(consumo))
                consumo_label.configure(text=f"Consumo médio encontrado: {consumo}W", text_color="green")
            else:
                consumo_label.configure(text="Não encontrei o consumo automaticamente", text_color="orange")
        else:
            consumo_label.configure(text="Digite o nome do equipamento primeiro", text_color="red")

    def adicionar_novo_dispositivo():  # Agora definida antes de ser usada
        nome = nome_entry.get()
        try:
    ...
    # 3. Finalmente construímos a interface
    ...
    # Botão para pesquisar consumo
    ctk.CTkButton(janela, 
                 text="Pesquisar Consumo Automaticamente", 
                 command=pesquisar_consumo, 
                 fg_color="#2d572c").pack(pady=5)
    
    consumo_label.pack()
    
    # Potência (em watts)
    ctk.CTkLabel(janela, text="Potência (em W):").pack()
    potencia_entry.pack(pady=5)
    
    # Botão de adicionar que usa a função agora definida
    ctk.CTkButton(janela, 
                 text="Adicionar", 
                 command=adicionar_novo_dispositivo).pack(pady=10)
...
def pesquisar_consumo_equipamento(nome_equipamento):
    """
    Pesquisa o consumo médio em watts de um equipamento no Google.
    Retorna o valor em watts ou None se não encontrar.
    """
    try:
        # Padrão regex para encontrar consumo em watts (ex: 100W, 50 watts, 1.5kW)
        consumo_pattern = re.compile(r'(\d+\.?\d*)\s?(?:W|watts|kW|kiloWatts)\b', re.IGNORECASE)
        
        # Pesquisa no Google (limita a 5 resultados para ser rápido)
        query = f"{nome_equipamento} consumo médio de energia eletrica em watts"
        resultados = search(query, num_results=20, lang="pt")
        ...
        for url in resultados:
            try:
                ...
                matches = consumo_pattern.findall(texto_pagina)
                if matches:
                    # Converte todos os valores para watts (transforma kW em W)
                    valores_w = []
                    for valor in matches:
                        valor_float = float(valor)
                        if 'kw' in texto_pagina.lower():  # Se encontrar kW no contexto
                            valor_float *= 1000  # Converte kW para W
                        valores_w.append(valor_float)
                    
                    # Retorna a média dos valores encontrados (arredondado)
                    if valores_w:
                        return round(sum(valores_w) / len(valores_w))
